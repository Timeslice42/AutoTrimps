...
function getBattleStats(what, form, crit) {
	...
	if (form && game.global.formation > 0) {
		...
		if ((game.global.formation == 1 && what == "health") || (game.global.formation == 2 && what == "attack") || (game.global.formation == 3 && what == "block"))
			formStrength = 4;
...
function calcOurDmg(minMaxAvg, incStance, incFlucts) {
	...
	if (critChance > 0) {
		...
		if (critTier > 0) {
			...
			if (critChance > 0)
				max *= getMegaCritDamageMult(critTier + 1);
			else
				max = min;
		}
...
function autoMap() {
	...
	if (voidMapLevelSettingMap.length == 1)
		voidMapLevelSettingMap += "0";
		...
		for (var map in game.global.mapsOwnedArray) {
			...
			if (theMap.noRecycle) {
				if (theMap.name == 'The Wall' && game.upgrades.Bounty.allowed == 0 && !game.talents.bounty.purchased) {
					...
					if (game.global.world < 15 + theMapDifficulty)
						continue;
				...
				if (theMap.name == 'Dimension of Anger' && document.getElementById("portalBtn").style.display == "none" && !game.talents.portal.purchased) {
					var theMapDifficulty = Math.ceil(theMap.difficulty / 2);
					if (game.global.world < 20 + theMapDifficulty)
						continue;
					selectedMap = theMap.id;
					break;
				}
				...
				if (theMap.name == 'The Block' && !game.upgrades.Shieldblock.allowed && ((game.global.challengeActive == "Scientist" || game.global.challengeActive == "Trimp") && !runningC2 || getPageSetting('BuyShieldblock'))) {
					var theMapDifficulty = Math.ceil(theMap.difficulty / 2);
					if (game.global.world < 11 + theMapDifficulty)
						continue;
					selectedMap = theMap.id;
					break;
				}
				...
				if (theMap.name == 'Trimple Of Doom' && (!runningC2 && game.mapUnlocks.AncientTreasure.canRunOnce && game.global.world >= treasure)) {
					var theMapDifficulty = Math.ceil(theMap.difficulty / 2);
					if ((game.global.world < 33 + theMapDifficulty) || treasure > -33 && treasure < 33)
						continue;
					selectedMap = theMap.id;
					if (treasure < 0)
				...
				if (!runningC2) {
					if (theMap.name == 'The Prison' && (game.global.challengeActive == "Electricity" || game.global.challengeActive == "Mapocalypse")) {
						var theMapDifficulty = Math.ceil(theMap.difficulty / 2);
						if (game.global.world < 80 + theMapDifficulty)
							continue;
						selectedMap = theMap.id;
						break;
					}
					if (theMap.name == 'Bionic Wonderland' && game.global.challengeActive == "Crushed") {
						var theMapDifficulty = Math.ceil(theMap.difficulty / 2);
						if (game.global.world < 125 + theMapDifficulty)
							continue;
						selectedMap = theMap.id;
						break;
					}
...
function updateAutoMapsStatus(a) {
	var b,
	c = getPageSetting('MinutestoFarmBeforeSpire');
	if (0 == getPageSetting('AutoMaps'))
		b = 'Off';
	else if ('Mapology' == game.global.challengeActive && 1 > game.challenges.Mapology.credits)
		b = 'Out of Map Credits';
	...
	if (Object.keys(mapSpecialModifierConfig).forEach(function (o) {
		...
		if (c) {
			if (59 <= game.global.highestLevelCleared) {
				...
				"0" != c.value && console.log("Set the map special modifier to: " + mapSpecialModifierConfig[c.value].name + ". Cost: " + (100 * (d / e)).toFixed(2) + "% of your fragments.")
			}
...
function mapTimeEstimater() {
	...
	return mapTimeEstimate = a && b ? a.currentTime - b.currentTime : 0,
	mapTimeEstimate
}
function HDratioy() {
	return HDratio
